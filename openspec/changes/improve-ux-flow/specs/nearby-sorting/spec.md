...
Con distancias disponibles, el sistema SHALL ordenar la lista de farmacias por distancia ascendente. Durante el mix nocturno, el orden por distancia SHALL aplicarse dentro de cada grupo horario, manteniendo la agrupación como criterio primario. Sin ubicación, la lista SHALL mantener el orden original del endpoint.
...
#### Scenario: El reorden no sigue al GPS en tiempo real
- **WHEN** la ubicación del usuario se actualiza por `watchPosition` durante un recorrido activo
- **THEN** el orden de la lista no cambia (el reorden ocurre al conceder ubicación o cambiar de fecha, no en cada actualización de GPS)
